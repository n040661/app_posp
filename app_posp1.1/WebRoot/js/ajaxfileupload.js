...
jQuery.extend({
    ...
    createUploadForm: function (ismore,id,contentType, fileElementId, data) {//id为当前系统时间字符串，fileElementId为页面<input type='file' />的id，data的值需要根据传入json的键来决定
        //将序列化转化为JSON
    	if(contentType!=null&&contentType=="serial"){
    ...
    ajaxFileUpload: function (s) {//这里s是个json对象，传入一些ajax的参数
        ...
        var form = jQuery.createUploadForm(s.isMore,id,s.contentType, s.fileElementId, (typeof (s.data) == 'undefined' ? false : s.data)); //创建动态form
    ...
    //$.paramJSON()json格式字符串转JSON对象
	//$.param()将JSON数组和对象序列化:serialize()=$.param(serializeArray())
	//将序列化格式转成JSON对象
     deserialize: function(serialize){
	      var s=$.trim(decodeURI(serialize));//解码URI并且去除空格
          /*replace("=",":")只替换扫描的第一个，可以根据indexOf("=")!=-1做While循环
	        while(ios_url.indexOf("\\")!=-1){
    	      ios_url=ios_url.replace("\\","/");
    	    } */
	      //s=s.replace(/=/g, ":");全局替换，replace(/=/gi, ":")忽略大小写
	      var a=s.split("&");
	      var oppsite=new Object();	      
	      for(var i=0;i<a.length;i++){
	           var o=a[i].split("=");
               eval("oppsite."+o[0]+"='"+o[1]+"'");//eval()能够处理字符串状态的JS及参数表达式；
               //字符串编译自加引号"oppsite."+o[0]+"
	           //alert(eval("oppsite."+o[0]));         
	      }
          return oppsite;
	  }
})
