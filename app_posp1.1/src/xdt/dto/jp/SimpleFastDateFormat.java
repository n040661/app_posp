...
public class SimpleFastDateFormat extends Format{
    ...
    private transient SimpleFastDateFormat.Rule[] mRules;
    ...
    public static synchronized SimpleFastDateFormat getDateInstance(int style, TimeZone timeZone, Locale locale) {
        ...
        if(timeZone != null) {
            key = new SimpleFastDateFormat.Pair(key, timeZone);
        }
        ...
        SimpleFastDateFormat.Pair key1 = new SimpleFastDateFormat.Pair(key, locale);
    ...
    public static synchronized SimpleFastDateFormat getTimeInstance(int style, TimeZone timeZone, Locale locale) {
        ...
        if(locale != null) {
            key = new SimpleFastDateFormat.Pair(key, locale);
        }
    ...
    public static synchronized SimpleFastDateFormat getDateTimeInstance(int dateStyle, int timeStyle, TimeZone timeZone, Locale locale) {
        SimpleFastDateFormat.Pair key = new SimpleFastDateFormat.Pair(new Integer(dateStyle), new Integer(timeStyle));
        if(timeZone != null) {
            key = new SimpleFastDateFormat.Pair(key, timeZone);
        }
    ...
    static synchronized String getTimeZoneDisplay(TimeZone tz, boolean daylight, int style, Locale locale) {
        SimpleFastDateFormat.TimeZoneDisplayKey key = new SimpleFastDateFormat.TimeZoneDisplayKey(tz, daylight, style, locale);
    ...
    protected void init() {
        ...
        this.mRules = (SimpleFastDateFormat.Rule[])((SimpleFastDateFormat.Rule[])((SimpleFastDateFormat.Rule[])rulesList.toArray(new SimpleFastDateFormat.Rule[rulesList.size()])));
        int len = 0;
        ...
        for(int i = 0; i < length; ++i) {
            ...
            switch(c) {
                case '\'':
                    ...
                    if(sub.length() == 1) {
                        rule = new SimpleFastDateFormat.CharacterLiteral(sub.charAt(0));
                    } else {
                        rule = new SimpleFastDateFormat.StringLiteral(sub);
                    }
                    break;
                case '(':
                case ')':
                ...
                case 'D':
                    ...
                    break;
                case 'E':
                    rule = new SimpleFastDateFormat.TextField(7, tokenLen < 4?shortWeekdays:weekdays);
                    break;
                case 'F':
                    rule = this.selectNumberRule(8, tokenLen);
                    break;
                case 'G':
                    rule = new SimpleFastDateFormat.TextField(0, ERAs);
                    break;
                case 'H':
                ...
                case 'M':
                    if(tokenLen >= 4) {
                        rule = new SimpleFastDateFormat.TextField(2, months);
                    } else if(tokenLen == 3) {
                        rule = new SimpleFastDateFormat.TextField(2, shortMonths);
                    } else if(tokenLen == 2) {
                        rule = SimpleFastDateFormat.TwoDigitMonthField.INSTANCE;
                    } else {
                        rule = SimpleFastDateFormat.UnpaddedMonthField.INSTANCE;
                    }
                    break;
                case 'S':
                ...
                case 'Z':
                    if(tokenLen == 1) {
                        rule = SimpleFastDateFormat.TimeZoneNumberRule.INSTANCE_NO_COLON;
                    } else {
                        rule = SimpleFastDateFormat.TimeZoneNumberRule.INSTANCE_COLON;
                    }
                    break;
                case 'a':
                    rule = new SimpleFastDateFormat.TextField(9, AmPmStrings);
                    break;
                case 'd':
                    rule = this.selectNumberRule(5, tokenLen);
                    break;
                case 'h':
                    rule = new SimpleFastDateFormat.TwelveHourField(this.selectNumberRule(10, tokenLen));
                    break;
                case 'k':
                    rule = new SimpleFastDateFormat.TwentyFourHourField(this.selectNumberRule(11, tokenLen));
                    break;
                case 'm':
                ...
                case 'y':
                    if(tokenLen >= 4) {
                        ...
                        rule = SimpleFastDateFormat.TwoDigitYearField.INSTANCE;
                    }
                    break;
                case 'z':
                    if(tokenLen >= 4) {
                        rule = new SimpleFastDateFormat.TimeZoneNameRule(this.mTimeZone, this.mTimeZoneForced, this.mLocale, 1);
                    } else {
                        rule = new SimpleFastDateFormat.TimeZoneNameRule(this.mTimeZone, this.mTimeZoneForced, this.mLocale, 0);
                    }
            }
    ...
    protected SimpleFastDateFormat.NumberRule selectNumberRule(int field, int padding) {
        switch(padding) {
            case 1:
                return new SimpleFastDateFormat.UnpaddedNumberField(field);
            case 2:
                return new SimpleFastDateFormat.TwoDigitNumberField(field);
            default:
                return new SimpleFastDateFormat.PaddedNumberField(field, padding);
        }
    }
    ...
    protected StringBuffer applyRules(Calendar calendar, StringBuffer buf) {
        SimpleFastDateFormat.Rule[] rules = this.mRules;
    ...
    private interface NumberRule extends SimpleFastDateFormat.Rule {
        void appendTo(StringBuffer var1, int var2);
    }

    private static class CharacterLiteral implements SimpleFastDateFormat.Rule {
    ...
    private static class StringLiteral implements SimpleFastDateFormat.Rule {
    ...
    private static class TextField implements SimpleFastDateFormat.Rule {
    ...
    private static class UnpaddedNumberField implements SimpleFastDateFormat.NumberRule {
        private final int mField;
    ...
    private static class UnpaddedMonthField implements SimpleFastDateFormat.NumberRule {
        static final SimpleFastDateFormat.UnpaddedMonthField INSTANCE = new SimpleFastDateFormat.UnpaddedMonthField();
    ...
    private static class PaddedNumberField implements SimpleFastDateFormat.NumberRule {
        private final int mField;
    ...
    private static class TwoDigitNumberField implements SimpleFastDateFormat.NumberRule {
        private final int mField;
    ...
    private static class TwoDigitYearField implements SimpleFastDateFormat.NumberRule {
        static final SimpleFastDateFormat.TwoDigitYearField INSTANCE = new SimpleFastDateFormat.TwoDigitYearField();
    ...
    private static class TwoDigitMonthField implements SimpleFastDateFormat.NumberRule {
        static final SimpleFastDateFormat.TwoDigitMonthField INSTANCE = new SimpleFastDateFormat.TwoDigitMonthField();
    ...
    private static class TwelveHourField implements SimpleFastDateFormat.NumberRule {
    ...
    private static class TwentyFourHourField implements SimpleFastDateFormat.NumberRule {
        private final SimpleFastDateFormat.NumberRule mRule;
    ...
    private static class TimeZoneNameRule implements SimpleFastDateFormat.Rule {
        private final TimeZone mTimeZone;
        private final boolean mTimeZoneForced;
        private final Locale mLocale;
    ...
    private static class TimeZoneNumberRule implements SimpleFastDateFormat.Rule {
        static final SimpleFastDateFormat.TimeZoneNumberRule INSTANCE_COLON = new SimpleFastDateFormat.TimeZoneNumberRule(true);
        static final SimpleFastDateFormat.TimeZoneNumberRule INSTANCE_NO_COLON = new SimpleFastDateFormat.TimeZoneNumberRule(false);
    ...
    private static class TimeZoneDisplayKey {
        ...
        public boolean equals(Object obj) {
            if(this == obj) {
                return true;
            } else if(!(obj instanceof SimpleFastDateFormat.TimeZoneDisplayKey)) {
                return false;
            } else {
                SimpleFastDateFormat.TimeZoneDisplayKey other = (SimpleFastDateFormat.TimeZoneDisplayKey)obj;
    ...
    private static class Pair {
        ...
        public boolean equals(Object obj) {
            if(this == obj) {
                return true;
            } else if(!(obj instanceof SimpleFastDateFormat.Pair)) {
                return false;
            } else {
                SimpleFastDateFormat.Pair key = (SimpleFastDateFormat.Pair)obj;
                if(this.mObj1 == null) {
                    if(key.mObj1 != null) {
                        return false;
                    }
