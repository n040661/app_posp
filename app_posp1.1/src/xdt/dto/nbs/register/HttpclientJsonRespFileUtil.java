			...
			try {
				...
				if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
					...
					try {
						...
						log.info("保存文件错误,path=" + path + "; error=" + e);
					} finally {
						try {
							if (bw != null)
								bw.close();
						} catch (Exception e) {
							log.info("finally BufferedOutputStream shutdown close: " + e);
						}
					}
				}
				// 如果失败
					...
					log.info("HttpResonse Error:" + errorMsg);

				}
			} catch (ClientProtocolException e) {
				System.out.println();
				log.info("下载文件保存到本地,http连接异常,path=" + path + ";error=" + e);
			} catch (IOException e) {
				log.info("下载文件保存到本地,文件操作异常,path=" + path + "; error=" + e);
			} finally {
				try {
					if (client != null) {
					...
					log.info("finally HttpClient shutdown error: " + e);
				}
			}
