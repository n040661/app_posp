...
// 解密由base64编码的私钥
     ...
     * @return
     */
    public static boolean verify(String source, String sign,String code,String TFB_PUBLIC_KEY) throws Exception {
        byte[] digest = sha1(source.getBytes(code));
     ...
     * @return
     */
    public static String encrypt(String paramstr,String code,String TFB_PUBLIC_KEY) {
        try {
            byte[] cipherData = RSAUtils.encryptByPublicKey(paramstr.getBytes(code),TFB_PUBLIC_KEY);
     ...
     */
    public static String decrypt(String cipherData,String code,String PRIVATE_KEY) {
        try {
            byte[] cipher = RSAUtils.decryptByPrivateKey(Base64.decode(cipherData), PRIVATE_KEY);
            String result = new String(cipher, code);
	 ...
	 * @throws Exception
	 */
	public static String encryptByPublicKey1(byte[] data, String publicKey) throws Exception {
		byte[] keyBytes = Base64.decode(publicKey);
		X509EncodedKeySpec x509KeySpec = new X509EncodedKeySpec(keyBytes);
		KeyFactory keyFactory = KeyFactory.getInstance(KEY_ALGORITHM);
		Key publicK = keyFactory.generatePublic(x509KeySpec);
	 ...
	 * @throws Exception
	 */
	public static String decryptByPrivateKey1(byte[] encryptedData, String privateKey) throws Exception {
		byte[] keyBytes = Base64.decode(privateKey);
		PKCS8EncodedKeySpec pkcs8KeySpec = new PKCS8EncodedKeySpec(keyBytes);
		KeyFactory keyFactory = KeyFactory.getInstance(KEY_ALGORITHM);
		Key privateK = keyFactory.generatePrivate(pkcs8KeySpec);
