...
public class ChanpayGatewayQpayDemo {
    ...
    public void cjt_query_trade() {
        ...
        origMap.put("version", "1.0");
        origMap.put("partner_id", Constant.MERCHANT_NO1); // 畅捷支付分配的商户号
        origMap.put("_input_charset", charset);// 字符集
        // 2.6查询交易接口
        origMap.put("outer_trade_no", "10303");// 这里有坑，是outer_trade_no【outer】不是out_trade_no【out】
        origMap.put("trade_type", "INSTANT");// 交易的类型类型包括 即时到账(INSTANT)，担保交易(ENSURE)， 退款(REFUND),提现（WITHDRAWAL）

        this.gatewayPost(origMap, charset, Constant.MERCHANT_PRIVATE_KEY1);
    }
    ...
    public void cjt_query_pay() {
        ...
        // 2.19快捷api确认
        origMap.put("outer_trade_no", "4a7841eaa2e34fff821c0eb451acb137");// 订单号
    ...
    public static void main(String[] args) {
        ...
        // test.createQPayConfirm();// 2.19快捷确认
           test.cjt_query_pay(); //2.30支付查询网关接口
    }
}
