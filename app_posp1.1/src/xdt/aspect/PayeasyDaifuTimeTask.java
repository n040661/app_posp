...
@Component
public class PayeasyDaifuTimeTask {
	Logger logger = Logger.getLogger(PayeasyDaifuTimeTask.class);
	...
	public void daifuTimeSelect() throws Exception {
		...
		List<PmsDaifuMerchantInfo> list = pmsDaifuMerchantInfoDao.selectDaifu();
			...
			if (!(merchantList.size() == 0 || merchantList.isEmpty())) {
				...
				if ("60".equals(merchantinfo.getMercSts())) {
					...
					if ("1".equals(status1)) {
						...
						pmsDaifuMerchantInfoDao.update(pdf);
						PmsDaifuMerchantInfo daifu=pmsDaifuMerchantInfoDao.selectByDaifuMerchantInfo(pdf);
						logger.info("代付的金额:"+daifu.getAmount());
						Double position=Double.parseDouble(merchantinfo.getPositionT1().toString());
						logger.info("可用额度:" + position);
						Double surAmount=position+Double.parseDouble(daifu.getAmount())*100;
						logger.info("剩余可用额度:" + surAmount);
						merchantinfo.setPositionT1(surAmount.toString());
						int num = this.pmsMerchantInfoDao.UpdatePmsMerchantInfo(merchantinfo);
						logger.info("修改额度"+num);
						//插入代付数据
						PmsDaifuMerchantInfo model = new PmsDaifuMerchantInfo();
						model.setMercId(daifu.getMercId());
						model.setCount("1");
						model.setIdentity("***");
						model.setBatchNo(pmsDaifuMerchantInfo.getBatchNo()+"/B");
						model.setAmount(Double.parseDouble(daifu.getAmount())/100 + "");
						model.setCardno("***");
						model.setRealname("***");
						model.setPayamount(Double.parseDouble(daifu.getAmount())+"");
						model.setPmsbankno("***");
						model.setTransactionType("代付补款");
						model.setPosition(surAmount.toString());
						model.setRemarks(model.getRemarks());
						model.setRecordDescription("批次号:" + model.getBatchNo() + "错误原因:" +"代付失败");
