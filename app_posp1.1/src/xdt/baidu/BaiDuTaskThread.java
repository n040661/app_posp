...
public class BaiDuTaskThread extends Thread {
	
	private Logger logger = Logger.getLogger(BaiDuTaskThread.class);
	...
	public BaiDuTaskThread(String order_no, IPmsAppTransInfoDao pmsAppTransInfoDao,ITAccAccountDao accountDao,
		...
		try {
			sleep(2000);
 			for(int i=0;i<times.length;i++){
 				logger.info("线程处理调用百度订单查询接口开始，时间："+UtilDate.getDateFormatter());
				//根据订单号获取到本地订单
				...
				if(appTransInfo!=null){
					if("0".equals(appTransInfo.getStatus())||"3".equals(appTransInfo.getStatus())){
						...
						baiduBackRequestDTO.setOrder_no(order_no);
						...
						merchantCollectMoneyServiceImpl.baiduHandelOrder(baiduBackRequestDTO);
				...
				logger.info("线程处理调用百度订单查询接口结束，时间："+UtilDate.getDateFormatter());	
			}
	        
		} catch (Exception e) {
			logger.debug("线程处理调用百度订单查询接口异常，时间："+UtilDate.getDateFormatter(),e);
		}
