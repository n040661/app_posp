...
public class XDTDesEncryptor
{
  ...
  public XDTDesEncryptor(byte[] userkey)
  {
    ...
    throws InvalidParameterException
  {
    ...
    {
      throw new InvalidParameterException("MPSE005");
    }

    init(true, this.userkey);

    int processTimes = idata.length / 8;
    int leftdata = idata.length % 8;
    int olen = 0;
    byte[] buf = new byte[8 * (processTimes + 1)];

    for (int i = 0; i < processTimes; ++i)
    {
      olen = (i + 1) * 8;
      processBlock(idata, olen - 8, buf, olen - 8);
    }
    ...
    for (int k = leftdata; k < 8; ++k) {
      temp[k] = (byte)(8 - leftdata);
    }
    ...
    throws InvalidParameterException
  {
    if ((idata == null) || (idata.length == 0))
    {
      throw new InvalidParameterException("MPSE006");
    }
    init(false, this.userkey);
